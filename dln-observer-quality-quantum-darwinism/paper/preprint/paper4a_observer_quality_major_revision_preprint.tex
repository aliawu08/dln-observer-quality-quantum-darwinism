...
(i) a decoder-level sample-complexity characterization for the number of environmental fragments needed to infer a \emph{binary} pointer value under an explicit access and calibration model,
(ii) a data-processing theorem showing that calibration noise (modeled as a CPTP channel) cannot increase the quantum Chernoff exponent, and
...
To anchor the framework in a concrete Hamiltonian model, we provide a fully worked open-system example: a central-spin pure-dephasing model whose conditional fragment states can be computed analytically, yielding explicit formulas for observer parameters as functions of couplings, readout noise, and acquisition time.
We also clarify the scope of ``local versus collective'' decoding claims:
for two pure hypotheses, optimal individual (non-entangling) strategies can match collective exponents and even the exact multi-copy Helstrom error \citep{AcinBaganBaigMasanesMunozTapia2005}, while commonly used \emph{fixed} per-fragment readouts (e.g., repeated single-copy Helstrom measurements) can incur a constant-factor penalty in the achievable exponent.
Finally, we show that the conditional-independence structure of SBS states is a bipartite factor DAG whose latent node is the pointer variable, enabling a coarse three-stage decoder classification $q_O\in\{q_{\mathrm{D}},q_{\mathrm{L}},q_{\mathrm{N}}\}$ and a revision-graph formalism governing adaptive transitions between measurement strategies under changing coherence conditions.
\end{abstract}
...
\begin{proposition}[Pure-state discrimination: decoder regimes and achievable exponents]
\label{prop:tight-gap}
Let $\rho_0=\ket{\psi_0}\!\bra{\psi_0}$ and $\rho_1=\ket{\psi_1}\!\bra{\psi_1}$ be pure states with overlap $c:=|\braket{\psi_0}{\psi_1}|\in(0,1)$, and consider discrimination of $m$ copies with equal priors.
Then:
\begin{enumerate}
\item (Dot) If the observer is restricted to a single copy ($q_{\mathrm{D}}$), the Bayes error does not decay exponentially with $m$; the corresponding asymptotic error exponent is $\xi_{\mathrm{D}}=0$.
\item (Network) Collective decoding ($q_{\mathrm{N}}$) achieves the quantum Chernoff exponent $\xi_{\mathrm{N}}=-\log(c^2)$ per copy.
\item (Linear) There is \emph{no universal exponent gap} between $q_{\mathrm{L}}$ and $q_{\mathrm{N}}$ for two pure hypotheses: optimal individual (non-entangling) strategies can achieve exponent $\xi_{\mathrm{L}}=\xi_{\mathrm{N}}$, and adaptive individual strategies can match the exact multi-copy Helstrom error for every $m$ \citep{AcinBaganBaigMasanesMunozTapia2005}.
\end{enumerate}
\end{proposition}

\begin{remark}[A useful restricted-$q_{\mathrm{L}}$ special case]
If one further restricts $q_{\mathrm{L}}$ to \emph{repeated single-copy Helstrom readout} (the Bayes-optimal measurement for one copy applied independently to each copy, followed by optimal classical postprocessing), the achievable exponent becomes $-\log c$ per copy, yielding an exact factor-of-two gap relative to $\xi_{\mathrm{N}}=-\log(c^2)$ (Cor.~\ref{cor:helstrom-penalty}).
\end{remark}
...
For discriminating $m$ copies with equal priors, there exist individual (non-entangling) measurement strategies whose error exponent matches the collective optimum, and adaptive individual strategies can match the exact multi-copy Helstrom error for every $m$ \citep{AcinBaganBaigMasanesMunozTapia2005}.
\end{proposition}
...
For two pure hypotheses, Proposition~\ref{prop:tight-gap} clarifies that there is no
\emph{universal} exponent separation between optimal individual and collective decoders.
However, under the practically motivated restriction to repeated single-copy Helstrom readout,
a clean constant-factor penalty arises (Cor.~\ref{cor:helstrom-penalty}), providing one concrete
QD instantiation of a DLN-style stage separation for pure-state evidence, in the spirit of
\citep[Proposition~1(i)]{WuDLNCompression2026preprint}.
The bounded-recovery result
...
The repeated-Helstrom restriction (Cor.~\ref{cor:helstrom-penalty}) provides a concrete scenario in which a plausible local readout choice leads to a provable constant-factor reduction in exponent relative to collective decoding. Temporal limits can motivate such readouts, but for two pure hypotheses they do not imply an exponent penalty by themselves.
...
Under additional local readout restrictions (e.g., repeated single-copy Helstrom measurements), one obtains a clean constant-factor penalty relative to collective decoding, providing a testable link between observer constraints and redundancy. For two pure hypotheses, optimal individual strategies can close this gap, highlighting the importance of stating measurement-class assumptions explicitly.
