...
def error_prob_product(xi_per_qubit: np.ndarray, m: int) -> float:
    """Product (Dec_L) error probability for m best fragments.

    This helper corresponds to a \emph{restricted} linear-stage decoder in which each
    fragment is measured immediately using the single-copy Helstrom POVM and the
    outcomes are classically aggregated.

    For two pure-state hypotheses, this fixed local readout yields a classical Chernoff
    exponent per copy of -log(c_k), so the aggregate exponent is
        xi_L = sum_{k in best m} (-log c_k) = xi_N / 2,
    where xi_N = sum_{k in best m} (-log c_k^2) is the collective (quantum Chernoff)
    exponent.

    Note: optimal individual measurement strategies can match the collective exponent
    for two pure hypotheses; xi_N/2 is specific to this repeated-Helstrom restriction.
    """
    xi_sorted = np.sort(xi_per_qubit)[::-1]
...
def pointer_resolution_gap(
    ...
    xi_total_L = np.empty_like(t_grid)  # restricted local (repeated-Helstrom) exponent
    ...
    # The gap (for this restricted local decoder): times where N resolves but L does not
...
def main() -> None:
    ...
    plot_xi_eff_vs_fcoh(
        xi_results,
        out_dir / "dynamical_redundancy_xi_eff.png",
        m=m,
        f_monitor=f_monitor,
    )
    ...
    plot_inverted_sophistication(
        cross,
        out_dir / "inverted_sophistication_crossover.png",
    )
    ...
    plot_pointer_resolution(
        res,
        out_dir / "pointer_resolution_gap.png",
        m=m_small,
        delta=delta,
    )
    ...
    plot_robustness_comparison(rob, out_dir / "robustness_decoherence_models.png")
